...
def resume_checkpoint(cfg, model, optimizer, scaler, criterion):
    ...
    if use_latest:
        ...
        if os.path.isfile(cfg.resume):
            ...
            logger.info("=> loaded resume checkpoint '{}'".format(cfg.resume))
...
#     """一个Dataset包装器，用于限制原始数据集到前N个样本"""
#     def __init__(self, dataset, n):
#         self.dataset = dataset
#         self.n = min(n, len(dataset))  # 防止n大于实际数据集的大小
...
#     val_dataset_limited = SubsetDataset(val_dataset, 20)
