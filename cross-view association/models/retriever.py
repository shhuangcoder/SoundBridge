...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, text, audio_features, env_features, use_checkpoint=False, apply_project=True):
        ...
        audio_embed = audio_features.unsqueeze(1)
        image_embed = x.unsqueeze(1)
        visual_audio_embed, _ = self.cross_attn_image(query=audio_embed, key=image_embed, value=image_embed)
        fusion_input = torch.cat([visual_audio_embed, audio_embed], dim=1)
        visual_audio_embed = self.cross_attn_fusion_visual(fusion_input)
        visual_audio_embed = torch.mean(visual_audio_embed, dim=1)
    ...
    def encode_text(self, text, text_audio, audio, use_checkpoint=False):
        ...
        fusion_input = torch.cat([visual_audio_embed, x2], dim=1)
        visual_audio_embed = self.cross_attn_fusion_text(fusion_input)
        visual_audio_embed = torch.mean(visual_audio_embed, dim=1)
