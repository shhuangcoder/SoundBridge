...
def main(args):
    ...
    if cfg.test.testonly:    
        ...
        logger.info('V2V Ego->Exo top1: {:.3f} | V2V Exo->Ego top1: {:.3f}'.format(metrics['Ego->Exo_top1'], metrics['Exo->Ego_top1']))
        logger.info('V2V Ego->Exo top5: {:.3f} | V2V Exo->Ego top5: {:.3f}'.format(metrics['Ego->Exo_top5'], metrics['Exo->Ego_top5']))
        logger.info('V2V Ego->Exo top10: {:.3f} | V2V Exo->Ego top10: {:.3f}'.format(metrics['Ego->Exo_top10'], metrics['Exo->Ego_top10']))
        exit(0)
    ...
    for epoch in range(start_epoch, cfg.train.epochs):
        ...
        logger.info('V2V Ego->Exo top1: {:.3f} | V2V Exo->Ego top1: {:.3f}'.format(v2v_metrics['Ego->Exo_top1'], v2v_metrics['Exo->Ego_top1']))
        logger.info('V2V Ego->Exo top5: {:.3f} | V2V Exo->Ego top5: {:.3f}'.format(v2v_metrics['Ego->Exo_top5'], v2v_metrics['Exo->Ego_top5']))
        logger.info('V2V Ego->Exo top10: {:.3f} | V2V Exo->Ego top10: {:.3f}'.format(v2v_metrics['Ego->Exo_top10'], v2v_metrics['Exo->Ego_top10']))
...
def train_one_epoch(train_loader, model, criterion, optimizer, scaler, epoch, lr_schedule, args, cfg, logger):
    ...
    for data_iter, inputs in enumerate(train_loader):
        ...
        with amp.autocast(enabled=not cfg.train.disable_amp):
            ...
            association_loss = loss_dict['loss']
            association_loss /= cfg.train.update_freq
...
def validate_v2v_mcq(val_loader, model, use_half=False, cfg=None, args=None, epoch=None, logger=None):
    ...
    with torch.no_grad():
        ...
        for i, inputs in enumerate(val_loader):
            ...
            audio_features = torch.mean(audio_features, dim=1)   #（1，1，768）->(1,768)
        ...
        save_pred_results(all_uids, all_preds, all_gts, cfg)
...
def save_pred_results(uids, preds, gts, scene_labels, cfg):
    import pandas as pd
    all_data = []
    ...
    for i in range(len(uids)):
        ...
        all_data.append([uid, prediction, gt, scene_labels])
    
    df = pd.DataFrame(all_data, columns=['uid', 'pred', 'GT', 'scene_label'])
